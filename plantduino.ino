...
const char* password = "TU_CLAVE_WIFI";  // Cambia por la contraseña de tu red WiFi
// Configura los parámetros de tu bot de Telegram
#define BOTtoken "#TOKEN_DE_TU_BOT"  // Token de tu bot
#define CHAT_ID "TU_ID_DE_TELEGRAM"  // Tu ID de chat de Telegram
...
// Constantes para el control del relé (lógica invertida)
#define RELE_ACTIVADO LOW     // IMPORTANTE: Asegúrate que coincida con tu relé
#define RELE_DESACTIVADO HIGH // IMPORTANTE: Asegúrate que coincida con tu relé
// GPIO12 (D6 en NodeMCU) para controlar la bomba
#define PIN_BOMBA 12
...
#define CAUDAL_BOMBA 7       // ml por segundo - ¡DEBES CALIBRAR ESTE VALOR!
...
void setup() {
  ...
  // CORRECCIÓN 1: Configura el pin del relé antes de cualquier otra cosa
  pinMode(PIN_BOMBA, OUTPUT);
  digitalWrite(PIN_BOMBA, RELE_DESACTIVADO);   // Asegurar que la bomba esté apagada al inicio
  Serial.println("Pin de la bomba configurado como OUTPUT y DESACTIVADO");
  
  pinMode(SENSOR_DIGITAL, INPUT);  // Para lectura digital
  
  // CORRECCIÓN 2: Inicialización separada de I2C y LCD para evitar conflictos
  Wire.begin(4, 5);  // SDA = GPIO4 (D2), SCL = GPIO5 (D1)
  delay(100);        // Pequeña pausa para estabilizar I2C
  ...
  // CORRECCIÓN 3: Verificación adicional del estado del relé
  Serial.println("Verificando estado inicial del relé...");
  Serial.print("Estado del pin PIN_BOMBA: ");
  Serial.println(digitalRead(PIN_BOMBA) == RELE_DESACTIVADO ? "DESACTIVADO" : "ACTIVADO");
...
// CORRECCIÓN 4: Función mejorada para activar la bomba
void activarBomba(int mililitros = RIEGO_DEFAULT) {
  ...
  // CORRECCIÓN: Verificación adicional antes de activar
  Serial.print("Estado actual del pin de la bomba antes de activar: ");
  Serial.println(digitalRead(PIN_BOMBA) == RELE_ACTIVADO ? "ACTIVADO" : "DESACTIVADO");
  
  // CORRECCIÓN: Asegurar que el pin esté configurado como salida
  pinMode(PIN_BOMBA, OUTPUT);
  
  // CORRECCIÓN: Activación explícita
  digitalWrite(PIN_BOMBA, RELE_ACTIVADO);  // Activar bomba
  
  // Verificación después de activar
  Serial.print("Estado del pin de la bomba después de activar: ");
  Serial.println(digitalRead(PIN_BOMBA) == RELE_ACTIVADO ? "ACTIVADO" : "DESACTIVADO");
...
// CORRECCIÓN 5: Función mejorada para desactivar la bomba
void desactivarBomba() {
  Serial.println("-------------------------------------------");
  Serial.println("DESACTIVANDO BOMBA DE AGUA");
  
  // CORRECCIÓN: Verificación adicional antes de desactivar
  Serial.print("Estado actual del pin de la bomba antes de desactivar: ");
  Serial.println(digitalRead(PIN_BOMBA) == RELE_ACTIVADO ? "ACTIVADO" : "DESACTIVADO");
  
  // CORRECCIÓN: Asegurar que el pin esté configurado como salida
  pinMode(PIN_BOMBA, OUTPUT);
  
  // CORRECCIÓN: Desactivación explícita
  digitalWrite(PIN_BOMBA, RELE_DESACTIVADO);  // Desactivar bomba
  
  // Verificación después de desactivar
  Serial.print("Estado del pin de la bomba después de desactivar: ");
  Serial.println(digitalRead(PIN_BOMBA) == RELE_ACTIVADO ? "ACTIVADO" : "DESACTIVADO");
...
// Función corregida para activar el modo viaje
void activarModoViaje(int umbralMin = 30, int umbralMax = 70) {
...
void calibrarSensor(int seco, int mojado) {
  valorSeco = seco;
  valorMojado = mojado;
